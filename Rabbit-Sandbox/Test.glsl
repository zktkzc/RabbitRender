...
layout (location = 2) out vec2 geo_TexCoords;
...
void main()
{
    ...
    gl_Position = ubo_View * vec4(vs_out.FragPos, 1.0);
}
...
uniform vec2 u_TestureOffset = vec2(0.0, 0.0);
...
void main()
{
    const vec4 diffuse = texture(u_DiffuseMap, u_TestureOffset + vec2(mod(fs_in.TexCoords.x * u_TextureTiling.x, 1), mod(fs_in.TexCoords.y * u_TextureTiling.y, 1))) * u_Diffuse;
